    ...
    <div class="row">
        <div class="col-12">
            <div class="mb-3 d-flex justify-content-between align-items-center">
                <h3 class="fw-bold mb-0"><?= __('အစားအစာစာရင်း') ?></h3>
            </div>

            <?php if (isset($_GET['msg']) && $_GET['msg'] == 'updated'): ?>
                <div class="alert alert-success alert-dismissible fade show" role="alert">
                    <i class="fas fa-check-circle me-2"></i>
                    <?= __('အစားအစာအချက်အလက်များ အောင်မြင်စွာ ပြင်ဆင်ပြီးပါပြီ။') ?>
                ...
                <table class="table table-hover table-bordered align-middle mb-0 glass-table">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th><?= __('အမည်') ?></th>
                            <th><?= __('ကလိုရီ') ?></th>
                            <th><?= __('ပရိုတိန်း') ?>(g)</th>
                            <th><?= __('ကာဗိုဟိုက်ဒြိတ်') ?>(g)</th>
                            <th><?= __('အဆီ') ?>(g)</th>
                            <th><?= __('ပမာဏ') ?></th>
                            <th><?= __('ယူနစ်') ?></th>
                        ...
                        <?php else: ?>
                            <?php while ($row = $result->fetch_assoc()): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="food_edit.php?id=<?= $row['id'] ?>" class="btn btn-sm btn-primary me-2" title="<?= __('ပြင်ဆင်မည်') ?>">
                                            <i class="bi bi-pencil"></i>
                                        </a>
                                        <button class="btn btn-sm btn-danger btn-delete" onclick="deleteFun(<?= $row['id'] ?>, 'foodId')" data-id="<?= $row['id'] ?>" title="<?= __('ဖျက်မည်') ?>">
